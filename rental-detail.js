...
function tourDays(calendar){
  ...
  const timeZone=zones[zoneKey]||zoneKey,now=localParts(new Date(),timeZone),today=`${now.year}-${String(now.month).padStart(2,'0')}-${String(now.day).padStart(2,'0')}`,anchor=new Date(Date.UTC(now.year,now.month-1,now.day,12)),days=[];
  for(let offset=0;offset<6;offset++){const date=new Date(anchor);date.setUTCDate(date.getUTCDate()+offset);const dateKey=date.toISOString().slice(0,10),range=calendar?.multiTimeRanges?.[String(date.getUTCDay())];if(!range?.active)continue;const times=[];for(const slot of range.slots||[]){let cursor=Math.floor(slot.startTime/100)*60+slot.startTime%100,end=Math.floor(slot.endTime/100)*60+slot.endTime%100;while(cursor<end){const value=Math.floor(cursor/60)*100+cursor%60;if(dateKey!==today||value>now.hour*100+now.minute)times.push(value);cursor+=duration}}if(times.length)days.push({date,dateKey,offset,times})}
  return{days,zoneKey,timeZone};
}
function renderTours(p,context){
  ...
  let chosenDay=0,chosenTime=null;
  const draw=()=>{const selected=days[chosenDay];scheduler.innerHTML=`<div class="tour-days" role="tablist" aria-label="Tour dates published by Aptly">${days.slice(0,5).map((day,i)=>`<button role="tab" aria-selected="${i===chosenDay}" class="${i===chosenDay?'is-selected':''}" data-tour-day="${i}"><span>${day.offset===0?'Today':day.offset===1?'Tomorrow':new Intl.DateTimeFormat('en-US',{weekday:'short',timeZone:'UTC'}).format(day.date)}</span><strong>${new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',timeZone:'UTC'}).format(day.date)}</strong></button>`).join('')}</div><p class="tour-time-note">Scheduling windows are loaded directly from Aptly. Final availability is confirmed when you reserve. Times shown in ${zoneLabels[schedule.zoneKey]||schedule.zoneKey}.</p><div class="tour-times">${selected.times.map(time=>`<button class="${time===chosenTime?'is-selected':''}" data-tour-time="${time}">${formatTime(time)}</button>`).join('')}</div><a class="gold-button tour-continue ${chosenTime==null?'is-disabled':''}" ${chosenTime==null?'aria-disabled="true"':`href="${esc(booking)}&tourDate=${encodeURIComponent(tourIso(selected.dateKey,chosenTime,schedule.timeZone))}" target="_blank" rel="noopener"`}>${chosenTime==null?'SELECT A TOUR TIME':`CONTINUE WITH ${formatTime(chosenTime)}`}</a>`};
  scheduler.addEventListener('click',e=>{const day=e.target.closest('[data-tour-day]');if(day){chosenDay=Number(day.dataset.tourDay);chosenTime=null;draw();return}const time=e.target.closest('[data-tour-time]');if(time){chosenTime=Number(time.dataset.tourTime);draw()}const disabled=e.target.closest('.tour-continue.is-disabled');if(disabled)e.preventDefault()});draw();
}
